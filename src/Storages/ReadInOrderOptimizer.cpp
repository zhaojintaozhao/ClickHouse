                                                                 ...
                                                                 const StorageMetadataPtr & metadata_snapshot) const
{
    ...
    if (dynamic_cast<const MergeTreeData *>(storage.get()) && metadata_snapshot->hasSortingKey())
    {
        sorting_key_columns = metadata_snapshot->getSortingKeyColumns();
    }
    else if (dynamic_cast<const StorageFromMergeTreeDataPart *>(storage.get()) && metadata_snapshot->hasSortingKey())
    {
        sorting_key_columns = metadata_snapshot->getSortingKeyColumns();
    }
